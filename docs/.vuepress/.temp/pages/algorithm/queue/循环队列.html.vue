<template><div><h1 id="队循环队列列" tabindex="-1"><a class="header-anchor" href="#队循环队列列"><span>队循环队列列</span></a></h1>
<h2 id="描述" tabindex="-1"><a class="header-anchor" href="#描述"><span>描述</span></a></h2>
<p>请你实现一个循环队列，该循环队列可利用的空间大小等于</p>
<p>n个int型变量的大小。
操作：
push x：将</p>
<p>x加入到循环队列尾端。若循环队列已满，输出&quot;full&quot;(不含引号)，否则不输出任何内容。保证</p>
<p>x为int型整数。
front：输出队首元素，队首不出队。若队列为空，输出&quot;empty&quot;(不含引号)。
pop：输出队首元素，且队首出队。若队列为空，输出&quot;empty&quot;(不含引号)。</p>
<h2 id="思路" tabindex="-1"><a class="header-anchor" href="#思路"><span>思路</span></a></h2>
<p>使用一个array，两个指针front，back表示队列，队列指针。
随着输入一直装
直到满，满的判断条件是 （back+1)%array.length = front</p>
